...
async function loadSongsFromList(fileList) {
  ...
  for (let i = 0; i < fileList.length; i++) {
    ...
    // Use GitHub Pages URL instead of raw URL to avoid CORS issues
...
async function play(i) {
  ...
  audio.src = song.file;
  audio.crossOrigin = "anonymous"; // Add this to help with CORS
  ...
  audio.play().then(() => {
    ...
    if (song.file.includes('2321564369.github.io')) {
      // If using GitHub Pages URL fails, try raw URL as fallback
